...
CACHE_FILE = 'data/cache_tr.json'
cache = {}
if os.path.exists(CACHE_FILE):
    try:
        with open(CACHE_FILE, 'r', encoding='utf-8') as f:
            cache = json.load(f)
        print(f"Loaded {len(cache)} cached Turkish translations.")
    except Exception:
        cache = {}
...
print(f"Unique words to translate into Turkish: {len(unique_to_translate)}")

def translate_item(word_str):
    t = GoogleTranslator(source='de', target='tr')
...
if unique_to_translate:
    print("Translating words into Turkish with thread pool...")
    count = 0
    ...
    with open(CACHE_FILE, 'w', encoding='utf-8') as f:
        json.dump(cache, f, ensure_ascii=False, indent=2)
    print("Turkish translations cached successfully.")

# Attach meaning_tr to all vocabulary entries
for item in vocab:
    w = item['word'].strip()
    item['meaning_tr'] = cache.get(w, w)
...
def save_csv(data, path):
    with open(path, 'w', encoding='utf-8-sig', newline='') as f:
        fieldnames = ['id', 'level', 'word', 'meaning_en', 'meaning_tr', 'full_entry', 'article', 'pos', 'plural', 'forms', 'examples', 'page']
...
# Save updated master and level-split CSVs with meaning_tr
save_csv(vocab, 'data/goethe_vocab.csv')
save_csv(vocab, 'data/goethe_vocab_tr.csv')

save_csv(a1_words, 'data/words_a1.csv')
save_csv(a1_words, 'data/words_a1_tr.csv')

save_csv(a2_words, 'data/words_a2.csv')
save_csv(a2_words, 'data/words_a2_tr.csv')

save_csv(b1_words, 'data/words_b1.csv')
save_csv(b1_words, 'data/words_b1_tr.csv')

# Copy all to public/data
for filename in [
    'goethe_vocab.json', 'words_a1.json', 'words_a2.json', 'words_b1.json',
    'goethe_vocab.csv', 'goethe_vocab_en.csv', 'goethe_vocab_tr.csv',
    'words_a1.csv', 'words_a1_en.csv', 'words_a1_tr.csv',
    'words_a2.csv', 'words_a2_en.csv', 'words_a2_tr.csv',
    'words_b1.csv', 'words_b1_en.csv', 'words_b1_tr.csv'
]:
    if os.path.exists(f'data/{filename}'):
        shutil.copy(f'data/{filename}', f'public/data/{filename}')

print("All datasets enriched with Turkish translations and synced to public/data!")
